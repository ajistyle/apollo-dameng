...
public interface ReleaseHistoryRepository extends PagingAndSortingRepository<ReleaseHistory, Long> {
  ...
  @Modifying
  @Query("update ReleaseHistory set IsDeleted = 1, DeletedAt = -Id, DataChange_LastModifiedBy = ?4 where appId=?1 and clusterName=?2 and namespaceName = ?3 and IsDeleted = 0")
