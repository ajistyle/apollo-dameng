...
public class PortalConfig extends RefreshableConfig {
  ...
  public List<Organization> organizations() {
    ...
    try {
      ...
      try {
        // 兼容 DB 中存了 \" 的旧数据（如达梦曾用反斜杠转义）
        String normalized = organizations.replace("\\\"", "\"");
        return GSON.fromJson(normalized, ORGANIZATION);
      } catch (Exception e2) {
        logger.warn("Failed to parse organizations config, return empty list. value: {}", organizations, e);
        return Collections.emptyList();
      }
    }
  }
