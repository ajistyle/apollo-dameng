...
## Adjust log dir if necessary (use /tmp for local if /opt/logs not writable)
LOG_DIR=${LOG_DIR:-/opt/logs/100003173}
...
# Pass APOLLO_PROFILE to JVM for Dameng (dm) or other profiles
if [ -n "$APOLLO_PROFILE" ]; then
  export JAVA_OPTS="$JAVA_OPTS -Dapollo_profile=$APOLLO_PROFILE"
fi
